...
public class PongScript : MonoBehaviour
{
    ...
    {
        if (message.id == id1) paddle1Position = new Vector2(-distanceFromCentre, -(message.y - 0.5f) * scale);
        else if (message.id == id2) paddle2Position = new Vector2(distanceFromCentre, -(message.y - 0.5f) * scale);
    }
